...
class LostItem(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('lost_items', cascade='all, delete-orphan'))
...
class FoundItem(db.Model):
	...
	description = db.Column(db.Text, nullable=True)
	date = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
	user = db.relationship('User', backref=db.backref('found_items', cascade='all, delete-orphan'))
	image = db.Column(db.String(20), nullable=False, default='default.jpg')
	filename = db.Column(db.String(length=30), nullable=False)
	resolved = db.Column(db.Boolean, default=False)
